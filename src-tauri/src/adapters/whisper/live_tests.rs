...
fn silence_and_near_silence_produce_no_text() {
    ...
    let hiss: Vec<f32> = (0..TARGET_SAMPLE_RATE as usize * 3)
        ...
        .collect();
    assert!(
        !is_digital_silence(&hiss),
        "this fixture must reach the model"
    );
    let segments = engine
...
fn tail_decode_is_measured_in_the_shape_a_session_produces_it() {
    ...
    //    HushWrite: it passes on an idle M4 Max and goes red on a laptop that
...
fn the_reduced_encoder_context_is_what_makes_the_tail_cheap() {
    ...
    let Some(speech) = testing::synthesise_speech("A short trailing fragment.", "fragment") else {
        eprintln!("skipped: could not synthesise a speech fixture");
        return;
    };
    if coreml_is_live() {
...
fn arabic_is_detected_and_returned_in_arabic_script() {
    ...
    assert!(
        has_arabic(&pinned_text),
        "pinning to Arabic must return Arabic script, got {pinned_text:?}"
    );
    assert!(
        has_arabic(&auto_text),
        "auto-detect must return Arabic script, got {auto_text:?}"
    );
}
